<?php
require '../db/db_connect.inc.php'; // Conexão com o banco de dados
require '../db/funcionario_crud.inc.php'; // Classe FuncionarioCRUD

// Iniciar a sessão antes de processar qualquer dado
session_start();

// Processar o login quando o formulário for enviado via POST
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $_SESSION['erro'] = "Por favor, insira um e-mail válido.";
        
    } else {
        // Tentar autenticar o funcionário
        $resultado = FuncionarioCRUD::autenticarFuncionario($email, $senha);

        if ($resultado['success']) {
            // Se o login for bem-sucedido, armazenar os dados na sessão
            $_SESSION['usuario_logado'] = [
                'id_funcionario' => $resultado['funcionario']['id_funcionario'],
                'nome' => $resultado['funcionario']['nome'],
                'email' => $resultado['funcionario']['email'],
                'admin' => $resultado['funcionario']['admin']
            ];

            // Redirecionar para a página de pesquisa após login
            header('Location: pesquisar.php');
            exit;
        } else {
            // Se a autenticação falhar, armazenar a mensagem de erro
            $_SESSION['erro'] = $resultado['message'];
            // Redirecionar para a página de login com a variável de erro na sessão
      ...
      <section>
        <div class="container-fluid wow fadeInUp" data-wow-delay="0.1s">
          <div class="container col-lg-9">
            ...
            <?php 
               // Em caso de erro, mostrar a mensagem armazenada na sessão
                if (isset($_SESSION['erro'])) {
                    echo "<p style='color: red;'>" . htmlspecialchars($_SESSION['erro']) . "</p>";
                    // Limpar a variável de erro após exibição
                    unset($_SESSION['erro']);
                    echo'<a class="btn btn-warning py-md-3 px-md-5 me-3" href="login.php"><i class="bi bi-arrow-left"></i> Tentar novamente</a>';
                }
            ?>
          </div>
        </div>
