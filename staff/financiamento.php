...
<head>
    <meta charset="UTF-8">
    <title>Pesquisar Financiamento</title>
            ...
            <div class="col-lg-12 text-center text-lg-start mb-2 mb-lg-0">
              <div class="d-inline-flex align-items-center" style="height: 45px;">
                <marquee scrollamount="6">
                  <small class="me-3 text-primary">Você está em uma área restrita do site. </small>
        ...
        <section>
            <div class="container-fluid py-5 wow fadeInUp" data-wow-delay="0.1s">
                <div class="container col-lg-9">
                    <h1>Resultados</h1>
                    <?php
                        // Incluir arquivos de conexão e funções CRUD
                        require '../db/db_connect.inc.php';
                        require '../db/cliente_crud.inc.php';
                        require '../db/veiculo_crud.inc.php';
                        require '../db/financiamento_crud.inc.php';
                        require '../domain/entities.inc.php';
                        ...
                        // Criar Cliente João
                        if (isset($_POST['criar_cliente_joao'])) {
                            $cliente = new Cliente(1, 'João Silva', 'joao@example.com', '11999999999', new DateTime());
                            $cliente_id = ClienteCRUD::create($cliente);
                            $resultado = "Cliente João criado com ID: $cliente_id";
                            echo "SUCESSO: CRIADO O CLIENTE JOÃO";
                        }
                        // Modificar Cliente para José
                        elseif (isset($_POST['modificar_cliente'])) {
                            $cliente = new Cliente(0, 'José Silva', 'jose@example.com', '11988888888', new DateTime());
                            ClienteCRUD::update($cliente);
                            $resultado = "Cliente modificado para José";
                        }
                        // Excluir Cliente
                        elseif (isset($_POST['excluir_cliente'])) {
                            ClienteCRUD::delete(1);
                            $resultado = "Cliente excluído";
                        }
                        
                        //-------------------------------------------------
                        
                        // Criar Veículo Fiat Uno 2025
                        elseif (isset($_POST['criar_veiculo'])) {
                            $veiculo = new Veiculo(0, 1, 'Fiat', 'Uno', 2025, 'XYZ1234');
                            $veiculo_id = VeiculoCRUD::create($veiculo);
                            $resultado = "Veículo Fiat Uno 2025 criado com ID: $veiculo_id";
                        }
                        // Consultar Veículos
                        elseif (isset($_POST['consultar_veiculo'])) {
                            $veiculos = VeiculoCRUD::readAll();
                            $resultado = 'Veículos consultados:<br>';
                            foreach ($veiculos as $veiculo) {
                                $resultado .= "ID: " . $veiculo['id_veiculo'] . " - Marca: " . $veiculo['marca'] . " - Modelo: " . $veiculo['modelo'] . "<br>";
                            }
                        }
                        // Modificar Veículo
                        elseif (isset($_POST['modificar_veiculo'])) {
                            $veiculo = new Veiculo(1, 1, 'Ford', 'Ka', 2015, 'ABC1234');
                            VeiculoCRUD::update($veiculo);
                            $resultado = "Veículo modificado para Ford Ka 2015";
                        }
                        // Excluir Veículo
                        elseif (isset($_POST['excluir_veiculo'])) {
                            VeiculoCRUD::delete(1);
                            $resultado = "Veículo excluído";
                        }
                        
                        //------------------------------------------------------
                        
                        // Criar Financiamento
                        elseif (isset($_POST['criar_financiamento'])) {
                            $financiamento = new Financiamento(0, 1, 20000, 48, 48, 500, 'ativo', new DateTime());
                            $financiamento_id = FinanciamentoCRUD::create($financiamento);
                            $resultado = "Financiamento criado com ID: $financiamento_id";
                        }
                        // Consultar Financiamentos
                        elseif (isset($_POST['consultar_financiamento'])) {
                            $financiamentos = FinanciamentoCRUD::readAll();
                            $resultado = 'Financiamentos consultados:<br>';
                            foreach ($financiamentos as $financiamento) {
                                $resultado .= "ID: " . $financiamento['id_financiamento'] . " - Valor Total: " . $financiamento['valor_total'] . " - Parcelas Restantes: " . $financiamento['parcelas_restantes'] . "<br>";
                            }
                        }
                        // Modificar Financiamento
                        elseif (isset($_POST['modificar_financiamento'])) {
                            $financiamento = new Financiamento(1, 1, 18000, 36, 36, 600, 'ativo', new DateTime());
                            FinanciamentoCRUD::update($financiamento);
                            $resultado = "Financiamento modificado";
                        }
                        // Excluir Financiamento
                        elseif (isset($_POST['excluir_financiamento'])) {
                            FinanciamentoCRUD::delete(1);
                            $resultado = "Financiamento excluído";
                        }
                    ...
                    <a href="teste_form.php">Voltar para o formulário</a>
                </div>
            </div>
        </section>
    </main>
